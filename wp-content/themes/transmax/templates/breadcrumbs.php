...
if (!class_exists('Transmax_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
    ...
    class Transmax_Breadcrumbs
    {
        ...
        {
            $this->text_home     = esc_html__('Home', 'transmax'); // text for the `Home` link
            $this->text_category = esc_html__('Archive by Category "%s"', 'transmax'); // text for a category page
            $this->text_search   = esc_html__('Search Results for "%s"', 'transmax'); // text for a search results page
            $this->text_tag      = esc_html__('Posts Tagged "%s"', 'transmax'); // text for a tag page
            $this->text_author   = esc_html__('Articles Posted by %s', 'transmax'); // text for an author page
            $this->text_404      = esc_html__('Error 404', 'transmax'); // text for the 404 page
            $this->text_page     = esc_html__('Page %s', 'transmax'); // text 'Page N'
            $this->text_cpage    = esc_html__('Comment Page %s', 'transmax'); // text `Comment Page N`
        }
        ...
        public function __construct ()
        {
            ...
            if (is_home()) {
	            ...
	            if( class_exists( 'WooCommerce' ) && ( is_product() || is_product_category() || is_product_tag() || is_cart() || is_checkout())){
		            ...
		            $out .= transmax_woocommerce_breadcrumb($args);
                ...
                // Portfolio Taxonomies Archive
                    ...
                    if (get_query_var('paged')) {
                        $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'transmax')) . $sep
                            . $before
                ...
                // Custom Post Type
                    ...
                    function_exists('is_product') && is_product()
                    && function_exists('transmax_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) { $out .= $sep; }
