    ...
    class WGL_Framework
    {
        ...
        public static function get_option(String $name)
        {
            if (class_exists('Redux')) {
                // Customizer
                if (!empty($GLOBALS['transmax_set'])) {
                    $theme_options = $GLOBALS['transmax_set'];
                } else {
                    $theme_options = get_option('transmax_set');
                }
            }

            if (empty($theme_options)) {
                $theme_options = get_option('transmax_default_options');
            }
        ...
        private static function _checkHex($hex)
        {
            ...
            if (strlen($color) == 3) {
                ...
                throw new Exception( esc_html__('HEX color needs to be 6 or 3 digits long', 'transmax') );
            }
        ...
        {
            $button_text = !empty($ajax_query_arr['load_more_text']) ? $ajax_query_arr['load_more_text'] : __('Load More', 'transmax');
        ...
        public static function get_sidebar_data($name = 'page')
        {
	        if ( is_front_page() && is_home() ) {
		        ...
		        $name = 'blog_list';  // Wordpress blog archive
	        }
        ...
        public static function posted_meta_on()
        {
            ...
            printf(
                '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'transmax') . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
            ...
            if ($metadata) {
                printf(
                    ...
                    esc_html_x('Full size', 'Used before full size attachment link.', 'transmax'),
            ...
            edit_post_link(
                ...
                sprintf(
                    wp_kses(__('Edit<span class="screen-reader-text"> "%s"</span>', 'transmax'), $kses_allowed_html),
                    get_the_title()
                ),
        ...
        public static function enqueue_css($style)
        {
            ...
            $css = ob_get_clean();
            $css = apply_filters('wgl_theme/enqueue_shortcode_css', $css, $style);
        }
