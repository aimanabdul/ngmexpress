...
(function( $ ) {
	...
	redux.field_objects.typography.select = function( selector, skipCheck, destroy, fontName, active ) {
		...
		style_multi = $( '#' + mainID + ' select.redux-typography-style-multi' ).val();
		...
		if ( active ) {
			...
			if ( ! that.hasClass( 'typography-initialized' ) ) {
				style_multi = $( '#' + mainID + ' select.redux-typography-style-multi' ).data( 'value' );
				...
				if ( style_multi !== "" ) {
					style_multi = String( style_multi );
				}
				style_multi_arr = style_multi.split(',');
				...
				if ( true === google ) {
					...
					$.each(
						...
						function( index, variant ) {
							...
							for (var i = 0; i < style_multi_arr.length; ++i) {
								if ( variant.id === style_multi_arr[i] ) {
									selected_multi = ' selected="selected"';
									break;
								} else {
									selected_multi = "";
								}
							...
							html_multi += '<option value="' + variant.id + '"' + selected_multi + '>' + variant.name.replace(
								/\+/g, " "
							) + '</option>';
						}
					);
					...
					if ( destroy ) {
						...
						that.find( '.redux-typography-style-multi' ).select2( 'destroy' );
					}

					// Insert new HTML.
					that.find( '.redux-typography-style' ).html( html ).select2();
					that.find( '.redux-typography-style-multi' ).html( html ).select2();
				...
				if ( details ) {
					...
					$.each(
						...
						function( index, value ) {
                            ...
                            html_multi += '<option value="' + index + '"' + selected_multi + '>' + value.replace(
                                '+', ' '
                            ) + '</option>';
						}
					);
		...
		if ( style_multi === '' || style_multi === undefined ) {
            that.find( '.typography-font-weight-multi' ).val( '' );
        } else {
            that.find( '.typography-font-weight-multi' ).val( style_multi );
        }
