	...
	class Redux_Repeater extends Redux_Field {
		...
		public function render() {
			...
			if ( isset( $this->value['redux_repeater_data'] ) && is_array( $this->value['redux_repeater_data'] ) && ! empty( $this->value['redux_repeater_data'] ) ) {
				...
				foreach ( $repeaters as $repeater ) {
					...
					if ( is_array( $repeater['title'] ) ) {
						$repeater['title'] = esc_html__( 'Title', 'wgl-extensions' );
					}
					...
					foreach ( $this->field['fields'] as $field ) {
						...
						if ( 'repeater' === $field['type'] || 'social_profiles' === $field['type'] || 'color_scheme' === $field['type'] ) {
							echo esc_html__( 'The', 'wgl-extensions' ) . ' <code>' . esc_html( $field['type'] ) . '</code> ' . esc_html__( 'field is not supported within the Repeater field.', 'wgl-extensions' );
						} else {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . esc_html__( 'Delete', 'wgl-extensions' ) . ' ' . esc_html( $this->field['item_name'] ) . '</a>';
					}
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . esc_attr( $disabled ) . '" data-name="' . esc_attr( $this->parent->args['opt_name'] . $this->repeater_values ) . '[title][]">' . esc_html__( 'Add', 'wgl-extensions' ) . ' ' . esc_html( $this->field['item_name'] ) . '</a><br/>';
			}
		}
		...
		public function localize( array $field, string $value = '' ) {
			...
			if ( isset( $field['fields'] ) && ! empty( $field['fields'] ) ) {
				...
				$var = array(
					'html'     => $var . '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . esc_html__( 'Delete', 'wgl-extensions' ) . '</a>',
					'count'    => count( $value ),
					'sortable' => true,
					'limit'    => '',
