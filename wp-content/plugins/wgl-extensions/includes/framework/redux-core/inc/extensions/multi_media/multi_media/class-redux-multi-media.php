...
if ( ! class_exists( 'Redux_Multi_Media' ) ) {
	...
	class Redux_Multi_Media extends Redux_Field {
		...
		public function render() {
			...
			$button_text    = $this->field['labels']['button'] ?? esc_html__( 'Add or Upload File(s)', 'wgl-extensions' );
			...
			if ( $this->value && is_array( $this->value ) ) {
				...
				foreach ( $this->value as $id => $url ) {
					...
					if ( $this->is_valid_img_ext( $url ) ) {
						...
						$html .= '<a href="#" class="redux_remove_file_button">' . esc_html__( 'Remove Image', 'wgl-extensions' ) . '</a>';
						$html .= '</p>';
						$html .= $id_input;
						$html .= '</li>';
						...
						// Add file to array.
						$html  = '<li>';
						$html .= esc_html__( 'File: ', 'wgl-extensions' ) . $id_input . '</li>';
						$html .= '<strong>' . $title . '</strong>&nbsp;&nbsp;&nbsp;';
						$html .= '(<a href="' . $url . '" target="_blank" rel="external">' . esc_html__( 'Download', 'wgl-extensions' ) . '</a> / <a href="#" class="redux_remove_file_button">' . __( 'Remove', 'wgl-extensions' ) . '</a>)';
						$html .= $id_input;
						$html .= '</li>';
		...
		public function enqueue() {
			...
			$upload_file    = $this->field['labels']['upload_file'] ?? esc_html__( 'Select File(s)', 'wgl-extensions' );
			$remove_image   = $this->field['labels']['remove_image'] ?? esc_html__( 'Remove Image', 'wgl-extensions' );
			$remove_file    = $this->field['labels']['remove_file'] ?? esc_html__( 'Remove', 'wgl-extensions' );
			$file_label     = $this->field['labels']['file'] ?? esc_html__( 'File: ', 'wgl-extensions' );
			$download_label = $this->field['labels']['download'] ?? esc_html__( 'Download', 'wgl-extensions' );
			$media_title    = $this->field['labels']['title'] ?? 'Title';

			// translators: %s: Filename.
			$dup_warn = $this->field['labels']['duplicate'] ?? esc_html__( '%s already exists in your file queue.', 'wgl-extensions' );

			// translators: %s: Upload limit.
			$max_warn = $this->field['labels']['max_limit'] ?? esc_html__( 'Maximum upload limit of %s reached/exceeded.', 'wgl-extensions' );
