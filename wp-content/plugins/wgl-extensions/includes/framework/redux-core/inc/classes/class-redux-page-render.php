	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'wgl-extensions' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'wgl-extensions' );
				}
				...
				$event = esc_html__( 'moving the mouse over', 'wgl-extensions' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'wgl-extensions' );
				}

				// Construct message.
				// translators: %1$s: Mouse action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'wgl-extensions' ), $event, Redux_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'redux-hint-tab',
					'title'   => esc_html__( 'Hints', 'wgl-extensions' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				// about hints.
				if ( true === $this->show_hints ) {
					$screen->set_help_sidebar( '<p><strong>Redux Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'wgl-extensions' ) . '</p>' );
				}
			}
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							echo sprintf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'wgl-extensions' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
		...
		private function get_default_output_string( array $field ): string {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'wgl-extensions' ) . ': ' . substr( $default_output, 0, - 2 );
			}
