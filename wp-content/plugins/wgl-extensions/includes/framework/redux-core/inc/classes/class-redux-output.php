		...
		public function enqueue() {
						...
						if ( isset( $field['type'] ) && 'callback' !== $field['type'] ) {
							...
							if ( ! empty( $core->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->can_output_css( $core, $field ) ) {
								...
								if ( null !== $style_data ) {
									...
									if (((isset($field['output']) && !empty($field['output'])) || (isset($field['compiler']) && !empty($field['compiler'])) || 'typography' === $field['type'] || 'custom_typography' === $field['type'] || 'icon_select' === $field['type'])) {
			...
			if ( ! empty( $core->typography ) && filter_var( $core->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
				...
				if (class_exists('ReduxFramework_custom_typography')) {
					$typography = new ReduxFramework_custom_typography(null, null, $this);
					$google_webfont = $typography->makeGoogleWebfontString2($core->typography);
				} else {
					$typography = new Redux_Typography(null, null, $core);
					$google_webfont = $typography->make_google_web_font_string($core->typography);
				}
				/* \ WGL CUSTOM TYPOGRAPHY FIX */
				...
				if ( $core->args['async_typography'] && ! empty( $core->typography ) ) {
					...
					<script>
						...
						WebFontConfig['google'] = {
							families: [<?php echo $google_webfont; // phpcs:ignore WordPress.Security.EscapeOutput ?>]
						};
