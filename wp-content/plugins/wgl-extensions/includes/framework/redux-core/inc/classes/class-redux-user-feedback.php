...
class Redux_User_Feedback {
	...
	public function seconds_to_words( string $seconds ): ?string {
		...
		if ( $years > 1 ) {
			...
			return sprintf( __( '%s years', 'wgl-extensions' ), $years );
		} elseif ( $years > 0 ) {
			return __( 'a year', 'wgl-extensions' );
		}
		...
		if ( $weeks > 1 ) {
			...
			return sprintf( __( '%s weeks', 'wgl-extensions' ), $weeks );
		} elseif ( $weeks > 0 ) {
			return __( 'a week', 'wgl-extensions' );
		}
		...
		if ( $days > 1 ) {
			...
			return sprintf( __( '%s days', 'wgl-extensions' ), $days );
		} elseif ( $days > 0 ) {
			return __( 'a day', 'wgl-extensions' );
		}
		...
		if ( $hours > 1 ) {
			...
			return sprintf( __( '%s hours', 'wgl-extensions' ), $hours );
		} elseif ( $hours > 0 ) {
			return __( 'an hour', 'wgl-extensions' );
		}
		...
		if ( $minutes > 1 ) {
			...
			return sprintf( __( '%s minutes', 'wgl-extensions' ), $minutes );
		} elseif ( $minutes > 0 ) {
			return __( 'a minute', 'wgl-extensions' );
		}
		...
		if ( $seconds > 1 ) {
			...
			return sprintf( __( '%s seconds', 'wgl-extensions' ), $seconds );
		} elseif ( $seconds > 0 ) {
			return __( 'a second', 'wgl-extensions' );
		}
			...
			<div class="notice updated redux-notice">
				<div class="redux-notice-inner">
					<div class="redux-notice-icon">
						...
						<img src="<?php echo esc_url( Redux_Core::$url . '/assets/img/icon--color.svg' ); ?>" alt="<?php printf( esc_attr__( '%s WordPress Plugin', 'wgl-extensions' ), esc_attr( $this->name ) ); ?>" />
					</div>
					<div class="redux-notice-content">
						<?php /* translators: 1. Name */ ?>
						<h3><?php printf( esc_html__( 'Are you enjoying %s?', 'wgl-extensions' ), esc_html( $this->name ) ); ?></h3>
						<p>
							<?php /* translators: 1. Name, 2. Time */ ?>
							<?php printf( esc_html__( 'You have been using %1$s for %2$s now. Would you mind leaving a review to let us know know what you think? We\'d really appreciate it!', 'wgl-extensions' ), esc_html( $this->name ), esc_html( $time ) ); ?>
						</p>
					</div>
					<div class="redux-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary redux-install-button" target="_blank">%2$s</a>', esc_url( 'https://wordpress.org/support/plugin/redux-framework/reviews/?filter=5#new-post' ), esc_html__( 'Leave a Review', 'wgl-extensions' ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', 'wgl-extensions' ); ?></a>
					</div>
				</div>
			</div>
			<?php
