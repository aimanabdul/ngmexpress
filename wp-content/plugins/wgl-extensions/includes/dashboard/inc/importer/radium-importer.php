 ...
 *
 *
 * Modified to work with Reduxframework/wgl_importer extension
 *
 */
...
if (!class_exists('Radium_Theme_Importer')) {
  ...
  class Radium_Theme_Importer
  {
    ...
    public function __construct()
    {
      ...
      $this->theme_options_file = isset($this->demo_custom_path) ? $this->demo_custom_path . $this->theme_options_file_name : $this->demo_files_path . $this->theme_options_file_name;
      $this->rev_slider_files   = isset($this->demo_custom_path) ? $this->demo_custom_path : $this->demo_files_path;
      $this->widgets            = isset($this->demo_custom_path) ? $this->demo_custom_path . $this->widgets_file_name : $this->demo_files_path . $this->widgets_file_name;
      $this->content_demo       = $this->demo_files_path . $this->content_demo_file_name;
      $this->type               = $this->import_type;
      $this->end                = $this->custom_end;
      $this->content_end        = $this->content_end;

      add_filter('add_post_metadata', array($this, 'check_previous_meta'), 10, 5);

      add_action('import_end', array($this, 'after_wp_importer'), 10, 1 );
    ...
    public function after_wp_importer( $atts = false )
    {

      $imported_demos = get_option('wgl_imported_demos');
      ...
      if ($this->part == '0') {
        do_action('wgl_importer_custom_page_slider', $atts);
        update_option('wgl_imported_demos', $imported_demos);
      }

      if ($this->part == '10') {
        do_action('wgl_importer_after_content_import', $this->active_import, $this->demo_files_path);
        update_option('wgl_imported_demos', $imported_demos);
      }
    }
    ...
    public function process_imports()
    {
      if (!empty($this->theme_options_file) && is_file($this->theme_options_file)) {
        ...
        if(in_array('options', $this->type) && !empty($this->content_end)){
          echo '<p>', esc_html('All done.'), ' <a href="', admin_url(), '">', esc_html('Have fun!'), '</a>' . '</p>';
        }
      }

      if (in_array('rev-slider', $this->type) && !empty($this->rev_slider_files) && !empty($this->end)) {
        if (class_exists('RevSlider')) {
          foreach (glob($this->rev_slider_files . '*.zip') as $file_name) {
            if (file_exists($file_name)) {
              $slider = new RevSlider();
              $slider->importSliderFromPost(true, true, $file_name);
            }
          }
        }
        echo '<p>', esc_html('All done.'), ' <a href="', admin_url(), '">', esc_html('Have fun!'), '</a>' . '</p>';
      ...
      if (!empty($this->content_demo) && is_file($this->content_demo) && empty($this->content_end)) {
      ...
      if (!empty($this->widgets) && is_file($this->widgets)) {
        ...
        if(in_array('widgets', $this->type) && !empty($this->content_end)){
          echo '<p>', esc_html('All done.'), ' <a href="', admin_url(), '">', esc_html('Have fun!') , '</a>' . '</p>';
        }
      }
    }
    ...
    public function set_demo_data($file)
    {
      ...
      if ($importer_error) {
        ...
        if (!is_file($file)) {
          ...
          $wp_import = new WGL_WP_Import();
          $wp_import->fetch_attachments = true;
          $wp_import->import($file, $this->end); 
        }
      }
    }
    ...
    public function set_demo_theme_options($file)
    {
      ...
      do_action('wgl_importer_after_theme_options_import', $this->active_import, $this->demo_files_path);
    }
    ...
    public function import_widgets($data)
    {
      ...
      foreach ($data as $sidebar_id => $widgets) {
        ...
        foreach ($widgets as $widget_instance_id => $widget) {
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = isset($widget->title) && $widget->title ? $widget->title : __('No Title', 'radium'); // show "No Title" if widget instance is untitled
